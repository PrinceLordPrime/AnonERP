 ...
 * @author Shan
 */
public class PAK_ISSUE_SALERTN_DB extends PAK_GLOBAL_DB{
    ...
    public String get_perqty_wrt_perdtl_code_seek(Connection conn, String a, String b,String c) {
        ...
        try
        {
            ...
            if(result.next()){
                cat = (Float.parseFloat(result.getString("SALEQTY"))+Float.parseFloat(result.getString("DMG"))+Float.parseFloat(result.getString("FREE"))-Float.parseFloat(result.getString("RETURNQTY")))+"";
            }else{
                cat=null;
            }
        } catch (SQLException ex) {
...
//PAID,REMARKS,NETPAYABLE,DCODE
             Sql="UPDATE ISMAIN SET AVDISCPER= '"+avdiscper+"', REFNO = '"+refno+"', CUSTCODE = '"+supcode+
                     ...
                     "',DCODE = '"+dealerCode+"',TOSUM = '"+tdosum+"',VANCODE = '"+van+"' WHERE DOCNO ="+docno;//
             statement.executeUpdate(Sql);
    ...
    public boolean insert_purchase_in_table(Connection conn,  String odate,String  docno,String  refno,String  supcode,
        ...
        {
            statement = conn.createStatement();
            String Sql = "INSERT INTO ISMAIN(ODATE,DOCNO,REFNO,CUSTCODE,TGROSSAMT,FMRSUM,SPLRS,OTHEREXP,AVDISCPER,"
...
//DOCNO,SEQNO,PCODE,SALEQTY,SALERATE,GROSSAMOUNT,DISCPER,TDO,TAXPER,FMR,NETAMOUNT,
            ...
            String Sql = "INSERT INTO ISDTL(DOCNO,SEQNO,PCODE,SALEQTY,SALERATE,GROSSAMOUNT,DISCPER,TDO,TAXPER,"
                    + "FMR,NETAMOUNT,RETURNQTY,DMG,FREE,CUSTOMSIZE) VALUES ('"+ Integer.parseInt(docno)+"','" + Integer.parseInt(seqno)+ "','"
                    ...
                    + "'"+dmg+"',"
                    + "'"+free+"',"
                    + "'"+customSize+"')";//+ "COSTPRICE = '"+getCPP(conn,pcode)+"'"
            //,,,,
            statement.executeUpdate(Sql);
//            }
            var = true;
...
//            DOCNO,SEQNO,PCODE,SALEQTY,SALERATE,GROSSAMOUNT,DISCPER,TDO,TAXPER,FMR,NETAMOUNT
    ...
    private ArrayList<String> getPurchase(ResultSet result) {
        ...
        try {
                ...
                String np4 = result.getString("VANCODE");
                cat.add(0,code );
                cat.add(1,c );
                cat.add(2,co );
                cat.add(3,ad2 );
...
//                cat=getPurchase(result);
            ...
            Logger.getLogger(PAK_ISSUE_SALERTN_DB.class.getName()).log(Level.SEVERE, null, ex);
        }finally{
            try{
		if (statement != null) statement.close();
		if (result != null) result.close();
